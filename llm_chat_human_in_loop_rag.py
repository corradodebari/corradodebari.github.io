...
def main():
    ...
    try:
        ...
        print(f"\nFull conversation log: {api_config.ui_host}/api/workflow/{workflow_id}")
        ...
        if assistant_task and assistant_task.input_data.get("prompt"):
            ...
            human_chat_collection = workflow_run.get_task(task_reference_name="collect_history_ref")
            if assistant_task and isinstance(assistant_task, list):
                human_chat_collection.output_data['result'].append(last_message)

            print(f"Conversation History: {json.dumps(human_chat_collection.output_data['result'], indent=2)}")
